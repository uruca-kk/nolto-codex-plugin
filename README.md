...
v0.1.0 では bundled MCP サーバー + 4 つのスキルが含まれます。Stop hook によるキューフラッシュは `templates/codex-hooks.json` をプロジェクトの `.codex/hooks.json` にコピーすることで有効化します（codex-cli 0.137.0 では plugin-bundled hooks が removed フラグになったため、プロジェクトフックとして同梱しています）。
...
`nolto.json` があると、スキル（Codex / AI ツール）は起動時にこのファイルを読み込み、すべての MCP 呼び出しで `projectId` を自動で明示します。複数のリポジトリを同一ユーザーで操作する場合でも、混在を防げます。
...
```json
{
  "hooks": {
    "Stop": [
      {
        "hooks": [
          { "type": "command", "command": "if command -v nolto >/dev/null 2>&1; then nolto flush --detach || true; else d=\"${XDG_CONFIG_HOME:-$HOME/.config}/nolto\"; if [ ! -f \"$d/.cli-hint-shown\" ]; then mkdir -p \"$d\" 2>/dev/null || true; : > \"$d/.cli-hint-shown\" 2>/dev/null || true; echo \"Nolto tip: install @nolto/cli (npm i -g @nolto/cli) to auto-send queued progress at session end. (shown once)\" >&2; fi; fi", "timeout": 10 }
        ]
      }
    ]
  }
...
> このコマンドは `@nolto/cli` 未導入時でも**エラーを出さず黙って no-op**（always exit 0）になります。初回のみ「CLI を入れると自動送信できる」ヒントを 1 回表示し、以降は無音です。

インストール済みプラグインのキャッシュからコピーする場合:

```bash
cp "$CODEX_HOME/plugins/cache/nolto/nolto/0.1.4/templates/codex-hooks.json" .codex/hooks.json
```
...
CLI 未導入時はフックがエラーを出さず no-op になります（上のヒント参照）。

### 動作フロー

Codex セッション中にモデルが `nolto queue <sub> <args>` を呼び出すと、進捗情報がプロジェクトの `.nolto/pending.jsonl` にオフラインで追記されます。セッション終了時に Stop フック (`codex-hooks.json`) が自動的に `nolto flush --detach` を実行します。
